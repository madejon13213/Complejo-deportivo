...
export default function ReservationCard({ reservation, onCancel }: ReservationCardProps) {
  ...
  const tone = normalized === "confirmada" || normalized === "pendiente" ? "neutral" : normalized === "cancelada" ? "error" : "success";

  const isCanceled = normalized === "cancelada";

  return (
    <article className="flex flex-col justify-between rounded-2xl border border-white/15 bg-black/35 p-4 text-white backdrop-blur-sm">
      <div>
