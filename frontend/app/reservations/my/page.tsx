...
import { Reservation, ReservationSearchResponse } from "@/lib/types";
...
export default function MyReservationsPage() {
  ...
  const reservationsQuery = useApiQuery<ReservationSearchResponse>(
    () => getReservationsByUser(numericUserId || 0, page, LIMIT, tab),
    [numericUserId, page, tab],
    { enabled: isReady && Boolean(numericUserId) }
  );

  const onTabChange = (newTab: ReservationTab) => {
    setTab(newTab);
    setPage(1);
  };
  ...
  const currentRows = reservationsQuery.data?.items || [];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4 p-4 md:p-8">
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant={tab === "activas" ? "primary" : "secondary"} onClick={() => onTabChange("activas")}>
          Activas / Próximas
        </Button>
        <Button variant={tab === "pasadas" ? "primary" : "secondary"} onClick={() => onTabChange("pasadas")}>
          Pasadas
        </Button>
        <Button variant={tab === "canceladas" ? "primary" : "secondary"} onClick={() => onTabChange("canceladas")}>
          Canceladas
        </Button>
      </div>
